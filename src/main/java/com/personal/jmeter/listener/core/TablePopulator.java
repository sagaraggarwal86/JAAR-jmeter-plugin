...
public final class TablePopulator {
     ...
     */
    public TablePopulator(DefaultTableModel tableModel,
                          JTable resultsTable,
     ...
     */
    public void populate(Map<String, SamplingStatCalculator> results,
    ...
    private void sortIfNeeded(List<Object[]> dataRows) {
        if (sortColumn < 0 || sortColumn >= ColumnIndex.ALL_COLUMNS.length) return;
     ...
     */
    public void handleHeaderClick(int viewCol, Runnable repopFn) {
        if (viewCol < 0) return;
     ...
     */
    public void storeOriginalColumns() {
     ...
     *
     * @param colIndex index into {@link ColumnIndex#ALL_COLUMNS}
     * @param visible  {@code true} to show, {@code false} to hide
     */
    public void toggleColumnVisibility(int colIndex, boolean visible) {
        TableColumnModel cm = resultsTable.getColumnModel();
    ...
    public List<String[]> getVisibleRows() {
        ...
        for (int r = 0; r < tableModel.getRowCount(); r++) {
            ...
            String[] cells = new String[ColumnIndex.ALL_COLUMNS.length];
            for (int c = 0; c < ColumnIndex.ALL_COLUMNS.length; c++) {
