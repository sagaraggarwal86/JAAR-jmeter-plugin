...
> A powerful, file-based Aggregate Report plugin for Apache JMeter with offset filtering, configurable percentile, column visibility controls, and CSV export.
...
2. Copy it to your JMeter `lib/ext/` directory:
   ```
   <JMETER_HOME>/lib/ext/jmeter-plugin-Config-Aggregate-Report-1.0.0.jar
   ```
3. Restart JMeter
...
| **Xth Percentile** | Configurable percentile (default: 90th) |
...
> **Note:** Duration may be slightly longer than `End Offset - Start Offset` because it includes the response time of the last sample within the offset window.
...
JMeter writes sub-results to JTL files as separate rows (e.g., `HTTP Request-0`, `HTTP Request-1`). The parser automatically detects and excludes these, matching the behavior of JMeter's built-in Aggregate Report. Only parent samples are aggregated.
...
The JAR is built to `target/jmeter-plugin-Config-Aggregate-Report-1.0.0.jar`.

**Deploy to JMeter:**
```bash
cp target/jmeter-plugin-Config-Aggregate-Report-1.0.0.jar $JMETER_HOME/lib/ext/
```
